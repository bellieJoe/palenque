...
namespace App\Livewire\Main\Users;
...
class UserIndex extends Component
{
    ...
    public function render()
    {
        $users = User::query()
            ...
            ->whereHas('roles', fn ($q) => $q->whereIn('role_type_id', [2, 3, 4, 5, 6]) && $q->where('municipal_market_id', auth()->user()->marketDesignation()->id))
            ->where(function ($q) {
                $q->where('name', 'like', '%' . $this->search . '%')
                ->orWhere('email', 'like', '%' . $this->search . '%');
            })
            ->paginate(10);
        return view('livewire.main.users.user-index', compact('users'));
    }
